...
window.addEventListener("load", () => {
    apikey = "dfae35b46b1c1c4a3ff4735b3661f180"
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition((position) => {
            ...
            const url = `http://api.openweathermap.org/data/2.5/weather?q=Pune&appid=${apikey}`;
...
document.getElementById('search').addEventListener('click', () => {
    ...
    var urlseach = `http://api.openweathermap.org/data/2.5/weather?q=${place}&appid=${apikey}`;
    fetch(urlseach).then(res => {
        return res.json()
